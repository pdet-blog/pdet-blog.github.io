...
(Scrooge)[https://github.com/pdet/Scrooge-McDuck] is an open-source, MIT license, DuckDB extension that provides functions and scanners aimed at easily integrating financial data analysis. I've recently added the capability to scan the Ethereum blockchain via the JSON-RPC protocol, and this blog post will cover its current capabilities and limitations. Note that the scanner is in an experimental state; both the API and overall performance are likely to change. It is not intended for production use!
...
All examples provided in this blogpost are available in a easy-to-run (collab example)[https://colab.research.google.com/drive/1vwX9Thnf11Odtx2U47oXRVo0eUmziHA0?usp=sharing].
